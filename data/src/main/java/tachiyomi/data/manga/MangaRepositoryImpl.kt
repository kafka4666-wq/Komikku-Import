...
class MangaRepositoryImpl(
    ...
    override suspend fun getFavoriteMangaPage(limit: Long, offset: Long): List<Manga> {
        val safeLimit = limit.coerceIn(1, 200)
        val safeOffset = offset.coerceAtLeast(0)
        return handler.awaitList {
            mangasQueries.getFavoriteMangaPage(safeLimit, safeOffset, MangaMapper::mapManga)
        }
    }

    override suspend fun getDuplicateLibraryEntries(limit: Long): List<Manga> {
        return handler.awaitList { mangasQueries.getDuplicateLibraryEntries(limit, MangaMapper::mapManga) }
    }
