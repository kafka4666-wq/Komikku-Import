...
class BackupCreator(
    ...
    suspend fun backup(uri: Uri, options: BackupOptions): String {
        ...
        try {
            ...
            // Build the input list once. The previous `a + b + c` chain created
            // multiple full-size temporary lists, which is expensive for very large libraries.
            val favorites = getFavorites.await()
            val nonFavoriteManga = if (options.readEntries) mangaRepository.getReadMangaNotInLibrary() else emptyList()
            // SY -->
            val mergedManga = getMergedManga.await()
            // SY <--
            val allManga = ArrayList<Manga>(favorites.size + nonFavoriteManga.size + mergedManga.size)
            allManga.addAll(favorites)
            allManga.addAll(nonFavoriteManga)
            // SY -->
            allManga.addAll(mergedManga)
            // SY <--
            val backupManga = backupMangas(allManga, options)
