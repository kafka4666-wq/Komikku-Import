...
class SyncDataJob(private val context: Context, workerParams: WorkerParameters) :
    ...
    override suspend fun doWork(): Result {
        ...
        return try {
            ...
            // A large library can exhaust the heap while building or merging sync data.
            // Keep the worker alive and report a compact, actionable failure instead of
            // handing the allocation error to the process-level crash screen.
            logcat(LogPriority.ERROR) { "Sync stopped because the device ran out of memory." }
            notifier.showSyncError("Sync stopped: not enough memory for the current library size")
            Result.success()
        } catch (e: Exception) {
            logcat(LogPriority.ERROR, e)
            notifier.showSyncError(e.message?.take(MAX_ERROR_TEXT_LENGTH) ?: e.javaClass.simpleName)
    ...
    companion object {
        ...
        private const val MAX_ERROR_TEXT_LENGTH = 256
