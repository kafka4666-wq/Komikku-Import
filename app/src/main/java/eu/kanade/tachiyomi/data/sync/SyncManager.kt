...
class SyncManager(
    ...
    suspend fun syncData() {
        ...
        // Keep only the fact needed later. Retaining 81k+ Manga objects through
        // backup creation, WebDAV merge, and upload multiplies peak heap use.
        val hasLocalManga = getAllMangaThatNeedsSync().isNotEmpty()
        ...
        val backupManga = backupCreator.backupMangas(getAllMangaThatNeedsSync(), backupOptions)
        val backup = Backup(
        ...
        if (syncPreferences.lastSyncTimestamp().get() == 0L && hasLocalManga) {
