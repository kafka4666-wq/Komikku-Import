...
class LibraryScreenModel(
    ...
    private suspend fun filterLibrary(unfiltered: List<LibraryItem>, query: String?, loggedInTrackServices: Map<Long, TriState>): List<LibraryItem> {
        return if (unfiltered.isNotEmpty() && !query.isNullOrBlank()) {
            // KMK -->
            // Explicit boolean operators opt into the bounded advanced-filter language.
            // Queries without AND/OR continue through the existing metadata-aware parser.
            if (query.contains(Regex("\\b(AND|OR)\\b", RegexOption.IGNORE_CASE))) {
                return unfiltered.asFlow().cancellable().filter { item ->
